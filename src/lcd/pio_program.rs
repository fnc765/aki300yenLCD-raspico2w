...
// 行間の CPU 介入（DMA 再設定 + SM1 への wait_push）が
// SM0 TX FIFO アンダーランを引き起こし、NCLK 一時停止 → 横ジッターが発生。
//
// ## Layer 7 の解決策
//
// SM0・SM1 の両方に独立した DMA チャネルを割り当て、
// フレーム全体を一括転送する:
//
// - DMA_CH0 → SM0: 拡張フレームバッファ 57,344 ワード (512×112)
// - DMA_CH1 → SM1: タイミングバッファ 225 ワード
// - `embassy_futures::join::join` で両 DMA を並行実行
//
// CPU 介入なし → FIFO アンダーラン不可 → ジッターゼロ。
//
// ## 拡張フレームバッファ (512×112)
//
// V-blank 期間 (16行) をフレームバッファ先頭に BLACK として含める:
//
// 行 0-15:   全ピクセル BLACK (V-blank)
// 行 16-111: [107 BLACK | 400 active | 5 BLACK] (アクティブ)
//
// ## SM1 タイミングバッファ (225 ワード)
//
// Layer 6 で CPU が wait_push していた SM1 供給データを
// 静的バッファに事前構築し、DMA_CH1 で一括転送する:
//
// [HSYNC_COUNT, VSYNC_REST_COUNT, NORMAL_LINES_Y,
//  HSYNC_COUNT, REST_COUNT, ... (× 111 通常ライン)]
//
// ## PIO プログラム
//
// SM0・SM1 の PIO プログラムは Layer 3 以降と同一。
// autopull (SM0) と pull block (SM1) の動作は変わらない。
// データ供給方式のみが CPU → DMA に変更される。
//
// ## メモリ
//
// 拡張 FB: 512×112×4 = 224 KB / バッファ
// ダブルバッファ: 448 KB / 520 KB SRAM (86%)
